...
void CL_UpdateTEnts()
{
	int i; //johnfitz -- use j instead of using i twice, so we don't corrupt memory
	beam_t* b;
	vec3_t dist, org;
	entity_t* ent;
	float yaw, pitch;
	...
	srand(static_cast<int>(cl.time * 1000)); //johnfitz -- freeze beams when paused
	...
	for (i = 0 , b = cl_beams; i < MAX_BEAMS; i++ , b++)
	{
		...
		{
			yaw = static_cast<int>(atan2(dist[1], dist[0]) * 180 / M_PI);
			if (yaw < 0)
				yaw += 360;
