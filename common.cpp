...
{
	auto buf = reinterpret_cast<byte*>(SZ_GetSpace(sb, 1));
	buf[0] = c;
}
...
{
	auto buf = reinterpret_cast<byte*>(SZ_GetSpace(sb, 2));
	buf[0] = c & 0xff;
	buf[1] = c >> 8;
}
...
{
	auto buf = reinterpret_cast<byte*>(SZ_GetSpace(sb, 4));
	buf[0] = c & 0xff;
	buf[1] = c >> 8 & 0xff;
	buf[2] = c >> 16 & 0xff;
	buf[3] = c >> 24;
...
{
	MSG_WriteShort(sb, f);
	MSG_WriteByte(sb, static_cast<int>(f * 255) % 255);
}
...
void COM_Init()
{
	...
	Cvar_RegisterVariable(&registered, nullptr);
	Cvar_RegisterVariable(&cmdline, nullptr);
...
pack_t* COM_LoadPackFile(char* packfile)
{
	...
	auto newfiles = reinterpret_cast<packfile_t*>(Z_Malloc(numpackfiles * sizeof(packfile_t)));
	//johnfitz
	...
	auto pack = reinterpret_cast<pack_t*>(Z_Malloc(sizeof (pack_t)));
	//johnfitz
...
void COM_AddGameDirectory(char* dir)
{
	...
	auto search = reinterpret_cast<searchpath_t*>(Z_Malloc(sizeof(searchpath_t)));
