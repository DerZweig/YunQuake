...
{
	int width = (vid.conwidth >> 3) - 2; //johnfitz -- use vid.conwidth instead of vid.width
	...
	auto numchars = oldwidth;

	if (con_linewidth < numchars)
		numchars = con_linewidth;

	auto mark = Hunk_LowMark(); //johnfitz
	auto tbuf = reinterpret_cast<char*>(Hunk_Alloc(con_buffersize)); //johnfitz
...
#define MAXGAMEDIRLEN	1000
	...
	con_text = reinterpret_cast<char*>(Hunk_AllocName(con_buffersize, "context"));//johnfitz -- con_buffersize replaces CON_TEXTSIZE
	Q_memset(con_text, ' ', con_buffersize);//johnfitz -- con_buffersize replaces CON_TEXTSIZE
	con_linewidth = -1;
	...
	//
	Cvar_RegisterVariable(&con_notifytime, nullptr);
	Cvar_RegisterVariable(&con_logcenterprint, nullptr); //johnfitz
...
{
	auto t = reinterpret_cast<tab_t*>(Hunk_Alloc(sizeof(tab_t)));
	t->name = name;
	t->type = type;
...
void Con_DrawNotify()
{
	...
	GL_SetCanvas(canvastype::CANVAS_CONSOLE); //johnfitz
	int v = vid.conheight; //johnfitz

	for (auto i = con_current - NUM_CON_TIMES + 1; i <= con_current; i++)
	{
		if (i < 0)
			continue;
		auto time = con_times[i % NUM_CON_TIMES];
		if (time == 0)
			continue;
		time = realtime - time;
...
void Con_DrawConsole(int lines, bool drawinput)
{
	...
	GL_SetCanvas(canvastype::CANVAS_CONSOLE);
