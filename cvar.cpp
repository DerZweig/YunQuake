...
void Cvar_Set(char* var_name, char* value)
{
	...
	if (!host_initialized)
	{
		...
		var->default_string = reinterpret_cast<char*>(Z_Malloc(Q_strlen(value) + 1));
...
void Cvar_RegisterVariable(cvar_t* variable, void (*function)())
{
	...
	variable->default_string = reinterpret_cast<char*>(Z_Malloc(Q_strlen(variable->string) + 1));
	...
	if (cvar_vars == nullptr || strcmp(variable->name, cvar_vars->name) < 0) //insert at front
	{
