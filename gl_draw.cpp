...
qpic_t* Draw_PicFromWad(char* name)
{
	...
	if (p->width < 64 && p->height < 64)
	{
		...
		texnum += scrap_texnum;
		gl->texnum = texnum;
		gl->sl     = (x + 0.01) / static_cast<float>(BLOCK_WIDTH);
		gl->sh     = (x + p->width - 0.01) / static_cast<float>(BLOCK_WIDTH);
		gl->tl     = (y + 0.01) / static_cast<float>(BLOCK_WIDTH);
		gl->th     = (y + p->height - 0.01) / static_cast<float>(BLOCK_WIDTH);
...
void Draw_Init()
{
	...
	if (!Q_strncasecmp(static_cast<const char *>(gl_renderer), "3dfx", 4) ||
		strstr(static_cast<const char *>(gl_renderer), "Glide"))
...
{
	if (x < 0 || static_cast<unsigned>(x + pic->width) > vid.width || y < 0 ||
		static_cast<unsigned>(y + pic->height) > vid.height)
	{
...
void Draw_ConsoleBackground(int lines)
{
	...
	if (lines > y)
		...
		Draw_AlphaPic(0, lines - vid.height, conback, static_cast<float>(1.2 * lines) / y);
}
...
{
	glColor3f(1, 1, 1);
	GL_Bind(*reinterpret_cast<int *>(draw_backtile->data));
	glBegin(GL_QUADS);
...
void GL_MipMap(byte* in, int width, int height)
{
	...
	{
		for (auto j = 0; j < width; j += 8, out += 4, in += 8)
		{
...
void GL_Upload32(unsigned* data, int width, int height, qboolean mipmap, qboolean alpha)
{
	...
	auto scaled_height = 1;
	while (scaled_height < height)
	{
		scaled_height <<= 1;
	}

	scaled_width >>= static_cast<int>(gl_picmip.value);
	scaled_height >>= static_cast<int>(gl_picmip.value);
...
void GL_Upload8_EXT(byte* data, int width, int height, qboolean mipmap, qboolean alpha)
{
	...
	if (mipmap)
	{
		...
		while (scaled_width > 1 || scaled_height > 1)
		{
			GL_MipMap8Bit(static_cast<byte *>(scaled), scaled_width, scaled_height);
			scaled_width >>= 1;
			scaled_height >>= 1;
			if (scaled_width < 1)
				scaled_width = 1;
