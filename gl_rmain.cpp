...
{
	float scale_forward = cos(DEG2RAD( angle ));
	float scale_side = sin(DEG2RAD( angle ));
...
void R_DrawViewModel()
{
	...
	if (currententity->model->type != modtype_t::mod_alias)
		return;
	//johnfitz
...
void R_ShowTris()
{
	...
	if (r_drawentities.value)
	{
		...
		if (r_drawviewmodel.value
			...
			&& currententity->model
			&& currententity->model->type == modtype_t::mod_alias)
		{
			glDepthRange(0, 0.3);
			R_DrawAliasModel_ShowTris(currententity);
			glDepthRange(0, 1);
...
void R_RenderView()
{
	...
	if (r_stereo.value)
	{
		auto eyesep = CLAMP(-8.0f, r_stereo.value, 8.0f);
		auto fdepth = CLAMP(32.0f, r_stereodepth.value, 1024.0f);
		...
		srand(static_cast<int>(cl.time * 1000)); //sync random stuff between eyes
	...
	if (r_speeds.value == 2)
		Con_Printf("%3i ms  %4i/%4i wpoly %4i/%4i epoly %3i lmap %4i/%4i sky %1.1f mtex\n",
		           static_cast<int>((time2 - time1) * 1000),
		           rs_brushpolys,
		           rs_brushpasses,
		           rs_aliaspolys,
		           rs_aliaspasses,
