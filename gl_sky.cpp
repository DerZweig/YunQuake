...
void Sky_LoadTexture(texture_t* mt)
{
	...
	solidskytexture = TexMgr_LoadImage(loadmodel, texturename, 128, 128, srcformat::SRC_INDEXED, back_data, "", reinterpret_cast<unsigned>(back_data), TEXPREF_NONE);
	...
	alphaskytexture = TexMgr_LoadImage(loadmodel, texturename, 128, 128, srcformat::SRC_INDEXED, front_data, "", reinterpret_cast<unsigned>(front_data), TEXPREF_ALPHA);
	...
	skyflatcolor[0] = static_cast<float>(r) / (count * 255);
	skyflatcolor[1] = static_cast<float>(g) / (count * 255);
	skyflatcolor[2] = static_cast<float>(b) / (count * 255);
}
...
void Sky_LoadSkyBox(char* name)
{
	...
	for (i = 0; i < 6; i++)
	{
		...
		if (data)
		{
			skybox_textures[i] = TexMgr_LoadImage(cl.worldmodel, filename, width, height, srcformat::SRC_RGBA, data, filename, 0, TEXPREF_NONE);
			nonefound = false;
		}
		else
		{
...
void Sky_NewMap()
{
	...
	//
	auto data = cl.worldmodel->entities;
	if (!data)
...
void Sky_Init()
{
	Cvar_RegisterVariable(&r_fastsky, nullptr);
	Cvar_RegisterVariable(&r_sky_quality, nullptr);
	Cvar_RegisterVariable(&r_skyalpha, nullptr);
	Cvar_RegisterVariable(&r_skyfog, nullptr);
...
void Sky_ProcessEntities()
{
	...
	for (auto i = 0; i < cl_numvisedicts; i++)
	{
		e = cl_visedicts[i];

		if (e->model->type != modtype_t::mod_brush)
			continue;
		...
		for (auto j = 0; j < e->model->nummodelsurfaces; j++ , s++)
		{
			if (s->flags & SURF_DRAWSKY)
			{
				auto dot = DotProduct (modelorg, s->plane->normal) - s->plane->dist;
					...
					!(s->flags & SURF_PLANEBACK) && dot > BACKFACE_EPSILON)
				{
					...
					auto mark = Hunk_LowMark();
					p = static_cast<glpoly_t*>(Hunk_Alloc(sizeof*s->polys)); //FIXME: don't allocate for each poly
...
void Sky_EmitSkyBoxVertex(float s, float t, int axis)
{
	...
	// avoid bilerp seam
	float w = skybox_textures[skytexorder[axis]]->width;
	float h = skybox_textures[skytexorder[axis]]->height;
	s = s * (w - 1) / w + 0.5 / w;
	t = t * (h - 1) / h + 0.5 / h;
...
void Sky_DrawFace(int axis)
{
	...
	p = static_cast<glpoly_t *>(Hunk_Alloc(sizeof(glpoly_t)));

	VectorSubtract(verts[2],verts[3],vup);
	VectorSubtract(verts[2],verts[1],vright);

	float di = max((int)r_sky_quality.value, 1);
	float qi = 1.0 / di;
