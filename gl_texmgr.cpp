...
void TexMgr_Anisotropy_f()
{
	...
	for (auto glt = active_gltextures; glt; glt = glt->next)
		TexMgr_SetFilterModes(glt);
}
...
void TexMgr_Imagelist_f()
{
	...
	auto mb = texels * (vid_bpp.value / 8.0f) / 0x100000;
	Con_Printf("%i textures %i pixels %1.1f megabytes\n", numgltextures, static_cast<int>(texels), mb);
}
...
void TexMgr_Imagedump_f()
{
	...
	for (auto glt = active_gltextures; glt; glt = glt->next)
	{
		...
		{
			buffer = static_cast<byte*>(malloc(glt->width * glt->height * 4));
			glGetTexImage(GL_TEXTURE_2D, 0, GL_RGBA, GL_UNSIGNED_BYTE, buffer);
			Image_WriteTGA(tganame, buffer, glt->width, glt->height, 32, true);
		}
		else
		{
			buffer = static_cast<byte*>(malloc(glt->width * glt->height * 3));
...
void TexMgr_FreeTextures(int flags, int mask)
{
	...
	for (auto glt = active_gltextures; glt; glt = next)
	{
		next = glt->next;
...
void TexMgr_LoadPalette()
{
	...
	auto pal = static_cast<byte*>(Hunk_Alloc(768));
	fread(pal, 1, 768, f);
	fclose(f);

	//standard palette, 255 is transparent
	auto dst = reinterpret_cast<byte *>(d_8to24table);
	auto src = pal;
	for (i = 0; i < 256; i++)
	{
		*dst++ = *src++;
	...
	d_8to24table[255] &= *reinterpret_cast<int *>(mask);

	//fullbright palette, 0-223 are black (for additive blending)
	src = pal + 224 * 3;
	dst = reinterpret_cast<byte *>(d_8to24table_fbright) + 224 * 4;
	for (i = 224; i < 256; i++)
	{
		*dst++ = *src++;
		*dst++ = *src++;
	...
	for (i = 0; i < 224; i++)
		d_8to24table_fbright[i] = *reinterpret_cast<int *>(black);

	//nobright palette, 224-255 are black (for additive blending)
	dst = reinterpret_cast<byte *>(d_8to24table_nobright);
	src = pal;
	for (i = 0; i < 256; i++)
	{
		*dst++ = *src++;
	...
	for (i = 224; i < 256; i++)
		d_8to24table_nobright[i] = *reinterpret_cast<int *>(black);

	//conchars palette, 0 and 255 are transparent
	memcpy(d_8to24table_conchars, d_8to24table, 256 * 4);
	d_8to24table_conchars[0] &= *reinterpret_cast<int *>(mask);
...
void TexMgr_RecalcWarpImageSize()
{
	...
	//
	auto mark = Hunk_LowMark();
	auto dummy = static_cast<byte *>(Hunk_Alloc(gl_warpimagesize * gl_warpimagesize * 4));
...
void TexMgr_Init()
{
	...
	// init texture list
	free_gltextures = static_cast<gltexture_t *>(Hunk_AllocName(MAX_GLTEXTURES * sizeof(gltexture_t), "gltextures"));
	...
	notexture = TexMgr_LoadImage(nullptr, "notexture", 2, 2, srcformat::SRC_RGBA,
	                             notexture_data, "", reinterpret_cast<unsigned>(notexture_data),
	                             TEXPREF_NEAREST | TEXPREF_PERSIST | TEXPREF_NOPICMIP);

	nullptrtexture = TexMgr_LoadImage(nullptr, "nullptrtexture", 2, 2, srcformat::SRC_RGBA,
	                                  nullptrtexture_data, "", reinterpret_cast<unsigned>(nullptrtexture_data),
	                                  TEXPREF_NEAREST | TEXPREF_PERSIST | TEXPREF_NOPICMIP);
...
unsigned* TexMgr_MipMapH(unsigned* data, int width, int height)
{
	...
	for (auto i = 0; i < height; i++ , in += width)
		for (auto j = 0; j < width; j += 4 , out += 4 , in += 4)
		{
...
unsigned* TexMgr_ResampleTexture(unsigned* in, int inwidth, int inheight, bool alpha)
{
	...
	for (auto i = 0; i < outheight; i++)
	{
		auto mody = y >> 8 & 0xFF;
		auto imody = 256 - mody;
		auto injump = (y >> 16) * inwidth;
		unsigned x = 0;

		for (auto j = 0; j < outwidth; j++)
		{
			auto modx = x >> 8 & 0xFF;
			auto imodx = 256 - modx;
...
void TexMgr_AlphaEdgeFix(byte* data, int width, int height)
{
	...
	{
		auto lastrow = width * 4 * (i == 0 ? height - 1 : i - 1);
		auto thisrow = width * 4 * i;
		auto nextrow = width * 4 * (i == height - 1 ? 0 : i + 1);

		for (auto j = 0; j < width; j++ , dest += 4)
		{
			if (dest[3]) //not transparent
				continue;

			auto lastpix = 4 * (j == 0 ? width - 1 : j - 1);
			auto thispix = 4 * j;
			auto nextpix = 4 * (j == width - 1 ? 0 : j + 1);

			auto b = lastrow + lastpix;
			if (data[b + 3])
			{
				c[0] += data[b];
				c[1] += data[b + 1];
			...
			if (n)
			{
				dest[0] = static_cast<byte>(c[0] / n);
				dest[1] = static_cast<byte>(c[1] / n);
				dest[2] = static_cast<byte>(c[2] / n);
...
void TexMgr_PadEdgeFixW(byte* data, int width, int height)
{
	...
	auto padw = TexMgr_PadConditional(width);
	auto padh = TexMgr_PadConditional(height);

	//copy last full column to first empty column, leaving alpha byte at zero
	auto src = data + (width - 1) * 4;
	for (i = 0; i < padh; i++)
	{
		src[4] = src[0];
		src[5] = src[1];
...
byte* TexMgr_PadImageW(byte* in, int width, int height, byte padbyte)
{
	...
	auto outwidth = TexMgr_Pad(width);

	auto out = static_cast<byte *>(Hunk_Alloc(outwidth * height));
	auto data = out;
...
byte* TexMgr_PadImageH(byte* in, int width, int height, byte padbyte)
{
	...
	auto srcpix = width * height;
	auto dstpix = width * TexMgr_Pad(height);

	auto out = static_cast<byte *>(Hunk_Alloc(dstpix));
	auto data = out;
	for (i = 0; i < srcpix; i++)
		*out++ = *in++;
	for (; i < dstpix; i++)
...
void TexMgr_LoadImage32(gltexture_t* glt, unsigned* data)
{
	...
	// mipmap down
	auto picmip = glt->flags & TEXPREF_NOPICMIP ? 0 : max ((int)gl_picmip.value, 0);
	auto mipwidth = TexMgr_SafeTextureSize(glt->width >> picmip);
	auto mipheight = TexMgr_SafeTextureSize(glt->height >> picmip);
	while (glt->width > mipwidth)
	{
		TexMgr_MipMapW(data, glt->width, glt->height);
		glt->width >>= 1;
		if (glt->flags & TEXPREF_ALPHA)
			TexMgr_AlphaEdgeFix(reinterpret_cast<byte *>(data), glt->width, glt->height);
	}
	...
	// upload
	GL_Bind(glt);
	auto internalformat = glt->flags & TEXPREF_ALPHA ? gl_alpha_format : gl_solid_format;
	...
	if (glt->flags & TEXPREF_MIPMAP)
	{
		...
		for (auto miplevel = 1; mipwidth > 1 || mipheight > 1; miplevel++)
		{
			if (mipwidth > 1)
			{
                              ...
                              byte* data, char* source_file, unsigned source_offset, unsigned flags)
{
	...
	switch (format)
	{
	case srcformat::SRC_INDEXED:
		crc = CRC_Block(data, width * height);
		break;
	case srcformat::SRC_LIGHTMAP:
...
void TexMgr_ReloadImage(gltexture_t* glt, int shirt, int pants)
{
	...
	if (glt->source_file[0] && glt->source_offset)
	{
		...
		data = Image_LoadImage(glt->source_file,
		                       reinterpret_cast<int*>(&glt->source_width),
		                       reinterpret_cast<int*>(&glt->source_height)); //simple file
	else if (!glt->source_file[0] && glt->source_offset)
		data = reinterpret_cast<byte *>(glt->source_offset); //image in memory
	...
	if (shirt > -1 && pants > -1)
	{
		if (glt->source_format == srcformat::SRC_INDEXED)
		{
			glt->shirt = shirt;
			glt->pants = pants;
		}
	...
	if (glt->shirt > -1 && glt->pants > -1)
	{
		...
		auto dst = translated = static_cast<byte *>(Hunk_Alloc(size));
		auto src = data;
