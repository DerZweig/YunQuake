...
unsigned short vid_systemgammaramp[768]; //to restore gamma on exits
int vid_gammaworks;
...
void VID_Restart()
{
	...
	{
		if (modelist[vid_default].type == modestate_t::MS_WINDOWED)
			mode_changed = true;
		else if (modelist[vid_default].bpp != static_cast<int>(vid_bpp.value))
			mode_changed = true;
		else if (modelist[vid_default].refreshrate != static_cast<int>(vid_refreshrate.value))
			mode_changed = true;
	}
	else if (modelist[vid_default].type != modestate_t::MS_WINDOWED)
		mode_changed = true;

	if (modelist[vid_default].width != static_cast<int>(vid_width.value) ||
		modelist[vid_default].height != static_cast<int>(vid_height.value))
		mode_changed = true;
	...
	if (mode_changed)
	{
		if (vid_fullscreen.value)
		{
			...
			{
				if (modelist[i].width == static_cast<int>(vid_width.value) &&
					modelist[i].height == static_cast<int>(vid_height.value) &&
					modelist[i].bpp == static_cast<int>(vid_bpp.value) &&
					modelist[i].refreshrate == static_cast<int>(vid_refreshrate.value))
				{
					break;
				}
			}
			...
			if (i == nummodes)
			{
				Con_Printf("%dx%dx%d %dHz is not a valid fullscreen mode\n",
				           static_cast<int>(vid_width.value),
				           static_cast<int>(vid_height.value),
				           static_cast<int>(vid_bpp.value),
				           static_cast<int>(vid_refreshrate.value));
				return;
			}
		...
		else //not fullscreen
		{
			...
			modelist[0].width = static_cast<int>(vid_width.value);
			modelist[0].height = static_cast<int>(vid_height.value);
		...
		if (!wglMakeCurrent(maindc, hrc))
		{
			...
			if (!wglMakeCurrent(maindc, hrc))
			{
				...
				FormatMessage(FORMAT_MESSAGE_ALLOCATE_BUFFER | FORMAT_MESSAGE_FROM_SYSTEM, nullptr, dw, MAKELANGID(LANG_NEUTRAL, SUBLANG_DEFAULT), reinterpret_cast<LPTSTR>(&lpMsgBuf), 0, nullptr);
				sprintf(szBuf, "VID_Restart: wglMakeCurrent failed with error %d: %s", dw, static_cast<const char*>(lpMsgBuf));
				Sys_Error(szBuf);
			}
			TexMgr_ReloadImages();
			GL_SetupState();
...
void VID_Test()
{
	...
	// now try the switch
	//
	auto oldmode = modelist[vid_default];
	...
	if (!SCR_ModalMessage("Would you like to keep this\nvideo mode? (y/n)\n", 5.0f))
	{
		...
		Cvar_Set("vid_fullscreen", oldmode.type == modestate_t::MS_WINDOWED ? "0" : "1");
		VID_Restart();
	}
}
...
char* GL_MakeNiceExtensionsList(const char* in)
{
	...
	for (i = 0 , count = 1; i < strlen(in); i++)
		if (in[i] == ' ')
			count++;
	auto out = static_cast<char*>(Z_Malloc(strlen(in) + count * 3 + 1)); //usually about 1-2k
	out[0] = 0;

	auto copy = static_cast<char*>(Z_Malloc(strlen(in) + 1));
	strcpy(copy, in);

	for (auto token = strtok(copy, " "); token; token = strtok(nullptr, " "))
	{
		strcat(out, "\n   ");
		strcat(out, token);
	}
...
void CheckArrayExtensions()
{
	...
	auto tmp = glGetString(GL_EXTENSIONS);
	while (*tmp)
	{
		if (strncmp(reinterpret_cast<const char*>(tmp), "GL_EXT_vertex_array", strlen("GL_EXT_vertex_array")) == 0)
		{
			if (
...
void GL_CheckExtensions()
{
	...
	{
		GL_MTexCoord2fFunc = reinterpret_cast<MTEXCOORDFUNC>(wglGetProcAddress("glMultiTexCoord2fARB"));
		GL_SelectTextureFunc = reinterpret_cast<SELECTTEXFUNC>(wglGetProcAddress("glActiveTextureARB"));
	...
	{
		GL_MTexCoord2fFunc = reinterpret_cast<MTEXCOORDFUNC>(wglGetProcAddress("glMTexCoord2fSGIS"));
		GL_SelectTextureFunc = reinterpret_cast<SELECTTEXFUNC>(wglGetProcAddress("glSelectTextureSGIS"));
		if (GL_MTexCoord2fFunc && GL_SelectTextureFunc)
		{
	...
	{
		wglSwapIntervalEXT = reinterpret_cast<SETSWAPFUNC>(wglGetProcAddress("wglSwapIntervalEXT"));
		wglGetSwapIntervalEXT = static_cast<GETSWAPFUNC>(wglGetProcAddress("wglGetSwapIntervalEXT"));
...
void GetWGLExtensions()
{
	...
	if ((wglGetExtensionsStringARB = reinterpret_cast<decltype(wglGetExtensionsStringARB)>(wglGetProcAddress("wglGetExtensionsStringARB"))))
		wgl_extensions = wglGetExtensionsStringARB(maindc);
	else if ((wglGetExtensionsStringEXT = reinterpret_cast<decltype(wglGetExtensionsStringEXT)>(wglGetProcAddress("wglGetExtensionsStringEXT"))))
...
{
	auto lastwidth = 0, lastheight = 0, lastbpp = 0, count = 0;
...
void VID_InitFullDIB(HINSTANCE hInstance)
{
	...
	do
	{
		for (auto j = 0; j < numlowresmodes && nummodes < MAX_MODE_LIST; j++)
		{
...
{
	int i, existingmode;
	int width, height = 0, bpp, findbpp;
	DEVMODE devmode;
	...
	HGLRC baseRC = wglCreateContext(maindc);
	if (!baseRC)
...
void VID_Menu_Init()
{
	...
	for (auto i = 1; i < nummodes; i++) //start i at mode 1 because 0 is windowed mode
	{
		auto w = modelist[i].width;
		auto h = modelist[i].height;
...
void VID_Menu_RebuildBppList()
{
	...
	{
		Cvar_SetValue("vid_bpp", static_cast<float>(modelist[0].bpp));
		return;
	}

	//if vid_bpp is not in the new list, change vid_bpp
	for (i = 0; i < vid_menu_numbpps; i++)
		if (vid_menu_bpps[i] == static_cast<int>(vid_bpp.value))
			break;

	if (i == vid_menu_numbpps)
		Cvar_SetValue("vid_bpp", static_cast<float>(vid_menu_bpps[0]));
}
...
void VID_Menu_RebuildRateList()
{
	...
	{
		Cvar_SetValue("vid_refreshrate", static_cast<float>(modelist[0].refreshrate));
		return;
	}

	//if vid_refreshrate is not in the new list, change vid_refreshrate
	for (i = 0; i < vid_menu_numrates; i++)
		if (vid_menu_rates[i] == static_cast<int>(vid_refreshrate.value))
			break;

	if (i == vid_menu_numrates)
		Cvar_SetValue("vid_refreshrate", static_cast<float>(vid_menu_rates[0]));
}
...
void VID_Menu_ChooseNextMode(int dir)
{
	...
	Cvar_SetValue("vid_width", static_cast<float>(vid_menu_modes[i].width));
	Cvar_SetValue("vid_height", static_cast<float>(vid_menu_modes[i].height));
	VID_Menu_RebuildBppList();
	VID_Menu_RebuildRateList();
	VID_Menu_CalcAspectRatio();
}
...
void VID_Menu_ChooseNextBpp(int dir)
{
	...
	Cvar_SetValue("vid_bpp", static_cast<float>(vid_menu_bpps[i]));
	VID_Menu_RebuildRateList();
}
...
void VID_Menu_ChooseNextRate(int dir)
{
	...
	Cvar_SetValue("vid_refreshrate", static_cast<float>(vid_menu_rates[i]));
}
...
void VID_MenuDraw()
{
	...
	// title
	char* title = "Video Options";
	M_PrintWhite((320 - 8 * strlen(title)) / 2, 32, title);

	// options
	M_Print(16, video_cursor_table[i], "        Video mode");
	M_Print(184, video_cursor_table[i], va("%ix%i (%i:%i)", static_cast<int>(vid_width.value), static_cast<int>(vid_height.value), vid_menu_rwidth, vid_menu_rheight));
	i++;

	M_Print(16, video_cursor_table[i], "       Color depth");
	M_Print(184, video_cursor_table[i], va("%i", static_cast<int>(vid_bpp.value)));
	i++;

	M_Print(16, video_cursor_table[i], "      Refresh rate");
	M_Print(184, video_cursor_table[i], va("%i Hz", static_cast<int>(vid_refreshrate.value)));
	i++;

	M_Print(16, video_cursor_table[i], "        Fullscreen");
	M_DrawCheckbox(184, video_cursor_table[i], static_cast<int>(vid_fullscreen.value));
	i++;
	...
	// cursor
	M_DrawCharacter(168, video_cursor_table[video_options_cursor], 12 + (static_cast<int>(realtime * 4) & 1));
