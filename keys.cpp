...
void Sys_GetClipboard()
{
	int i;

	if (OpenClipboard(nullptr))
	{
		auto th = GetClipboardData(CF_TEXT);
		if (th)
		{
			auto clipText = reinterpret_cast<char*>(GlobalLock(th));
			if (clipText)
			{
...
void Key_Console(int key)
{
	...
	case K_HOME:
		if (keydown[K_CTRL])
		{
			...
			{
				auto line = con_text + i % con_totallines * con_linewidth;
...
int Key_StringToKeynum(char* str)
{
	...
	for (auto kn = keynames; kn->name; kn++)
	{
...
void Key_SetBinding(int keynum, char* binding)
{
	...
	auto str = reinterpret_cast<char*>(Z_Malloc(l + 1));
	Q_strcpy(str, binding);
	str[l] = 0;
	keybindings[keynum] = str;
}
