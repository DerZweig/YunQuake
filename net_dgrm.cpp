...
void NET_Ban_f()
{
	...
	{
	case 1:
		if (reinterpret_cast<struct in_addr *>(&banAddr)->s_addr)
		{
			Q_strcpy(addrStr, inet_ntoa(*reinterpret_cast<struct in_addr *>(&banAddr)));
			Q_strcpy(maskStr, inet_ntoa(*reinterpret_cast<struct in_addr *>(&banMask)));
			print("Banning %s [%s]\n", addrStr, maskStr);
		}
		else
...
static void Test_Poll(void*)
{
	...
	while (true)
	{
		...
		MSG_ReadByte(); //byte playerNumber = 
		...
		for (n = 0; n < hostCacheCount; n++)
			if (Q_strcasecmp(host, hostcache[n].name) == 0)
			{
				if (hostcache[n].driver != myDriverLevel)
					continue;
				net_landriverlevel = hostcache[n].ldriver;
				Q_memcpy(&sendaddr, &hostcache[n].addr, sizeof(qsockaddr));
				break;
			}
		if (n < hostCacheCount)
			goto JustDoIt;
...
static qsocket_t* _Datagram_CheckNewConnections()
{
	...
	{
		unsigned long testAddr = reinterpret_cast<struct sockaddr_in *>(&clientaddr)->sin_addr.s_addr;
...
static void _Datagram_SearchForHosts(bool xmit)
{
	...
	while ((ret = dfunc.Read(dfunc.controlSock, net_message.data, net_message.maxsize, &readaddr)) > 0)
	{
		...
		MSG_BeginReading();
		int control = BigLong(*reinterpret_cast<int *>(net_message.data));
		MSG_ReadLong();
		if (control == -1)
			continue;
		if ((control & ~NETFLAG_LENGTH_MASK) != NETFLAG_CTL)
