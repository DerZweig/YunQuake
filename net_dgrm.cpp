...
void NET_Ban_f (void)
{
	...
	{
		case 1:
			if (((in_addr_t *)&banAddr)->s_addr)
			{
				Q_strcpy(addrStr, inet_ntoa(*(in_addr_t *)&banAddr));
				Q_strcpy(maskStr, inet_ntoa(*(in_addr_t *)&banMask));
				print("Banning %s [%s]\n", addrStr, maskStr);
			}
			else
...
static qsocket_t *_Datagram_CheckNewConnections (void)
{
	...
	if (clientaddr.sa_family == AF_INET)
	{
		...
		testAddr = ((sockaddr_in_t *)&clientaddr)->sin_addr.s_addr;
