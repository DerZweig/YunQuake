...
static qsockaddr broadcastaddr;

extern unsigned int winsock_initialized;
...
char* WIPX_AddrToString(qsockaddr* addr)
{
	...
	sprintf(buf, "%02x%02x%02x%02x:%02x%02x%02x%02x%02x%02x:%u",
	        reinterpret_cast<sockaddr_ipx *>(addr)->sa_netnum[0] & 0xff,
	        reinterpret_cast<sockaddr_ipx *>(addr)->sa_netnum[1] & 0xff,
	        reinterpret_cast<sockaddr_ipx *>(addr)->sa_netnum[2] & 0xff,
	        reinterpret_cast<sockaddr_ipx *>(addr)->sa_netnum[3] & 0xff,
	        reinterpret_cast<sockaddr_ipx *>(addr)->sa_nodenum[0] & 0xff,
	        reinterpret_cast<sockaddr_ipx *>(addr)->sa_nodenum[1] & 0xff,
	        reinterpret_cast<sockaddr_ipx *>(addr)->sa_nodenum[2] & 0xff,
	        reinterpret_cast<sockaddr_ipx *>(addr)->sa_nodenum[3] & 0xff,
	        reinterpret_cast<sockaddr_ipx *>(addr)->sa_nodenum[4] & 0xff,
	        reinterpret_cast<sockaddr_ipx *>(addr)->sa_nodenum[5] & 0xff,
	        ntohs(reinterpret_cast<sockaddr_ipx *>(addr)->sa_socket)
	);
	return buf;
}
...
int WIPX_AddrCompare(qsockaddr* addr1, qsockaddr* addr2)
{
	...
	if (*reinterpret_cast<sockaddr_ipx *>(addr1)->sa_netnum && *reinterpret_cast<sockaddr_ipx *>(addr2)->sa_netnum)
		if (memcmp(reinterpret_cast<sockaddr_ipx *>(addr1)->sa_netnum, reinterpret_cast<sockaddr_ipx *>(addr2)->sa_netnum, 4) != 0)
			return -1;
	if (memcmp(reinterpret_cast<sockaddr_ipx *>(addr1)->sa_nodenum, reinterpret_cast<sockaddr_ipx *>(addr2)->sa_nodenum, 6) != 0)
		return -1;

	if (reinterpret_cast<sockaddr_ipx *>(addr1)->sa_socket != reinterpret_cast<sockaddr_ipx *>(addr2)->sa_socket)
		return 1;
