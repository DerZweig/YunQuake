...
void PR_PrintStatement(dstatement_t* s)
{
	...
	{
		Con_Printf("%s ", pr_opnames[static_cast<int>(s->op)]);
		int i = strlen(pr_opnames[static_cast<int>(s->op)]);
		for (; i < 10; i++)
			Con_Printf(" ");
	}

	if (s->op == opcode_t::OP_IF || s->op == opcode_t::OP_IFNOT)
		Con_Printf("%sbranch %i", PR_GlobalString(s->a), s->b);
	else if (s->op == opcode_t::OP_GOTO)
	{
		Con_Printf("branch %i", s->a);
	}
	else if (static_cast<unsigned>(static_cast<int>(s->op) - static_cast<int>(opcode_t::OP_STORE_F)) < 6)
	{
