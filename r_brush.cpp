...
texture_t* R_TextureAnimation(texture_t* base, int frame)
{
	...
	auto relative = static_cast<int>(cl.time * 10) % base->anim_total;
...
void R_DrawSequentialPoly(msurface_t* s)
{
	...
	auto t = R_TextureAnimation(s->texinfo->texture, currententity->frame);
	auto entalpha = ENTALPHA_DECODE(currententity->alpha);
	...
	for (int i = 0; i < clmodel->nummodelsurfaces; i++ , psurf++)
	{
			...
			!(psurf->flags & SURF_PLANEBACK) && dot > BACKFACE_EPSILON)
		{
			if (psurf->flags & SURF_DRAWTURB && r_oldwater.value)
				for (glpoly_t* p = psurf->polys->next; p; p = p->next)
					DrawGLTriangleFan(p);
			else
...
void GL_BuildLightmaps()
{
	...
	for (i = 0; i < MAX_LIGHTMAPS; i++)
	{
		...
		byte* data = lightmaps + i * BLOCK_WIDTH * BLOCK_HEIGHT * lightmap_bytes;
		lightmap_textures[i] = TexMgr_LoadImage(cl.worldmodel, name, BLOCK_WIDTH, BLOCK_HEIGHT,
		                                        srcformat::SRC_LIGHTMAP, data, "",
		                                        reinterpret_cast<unsigned>(data), TEXPREF_LINEAR | TEXPREF_NOPICMIP);
		//johnfitz
	}
...
void R_AddDynamicLights(msurface_t* surf)
{
	...
	for (lnum = 0; lnum < MAX_DLIGHTS; lnum++)
	{
		...
		auto dist = DotProduct (cl_dlights[lnum].origin, surf->plane->normal) -
			surf->plane->dist;
		rad -= fabs(dist);
		...
		auto bl = blocklights;
		auto cred = cl_dlights[lnum].color[0] * 256.0f;
		auto cgreen = cl_dlights[lnum].color[1] * 256.0f;
		auto cblue = cl_dlights[lnum].color[2] * 256.0f;
		//johnfitz
		for (auto t = 0; t < tmax; t++)
		{
			...
			for (auto s = 0; s < smax; s++)
			{
				...
				//johnfitz -- lit support via lordhavoc
				{
					auto brightness = rad - dist;
					bl[0] += static_cast<int>(brightness * cred);
					bl[1] += static_cast<int>(brightness * cgreen);
					bl[2] += static_cast<int>(brightness * cblue);
				}
				bl += 3;
				//johnfitz
			}
...
void R_UploadLightmap(int lmap)
{
	...
	auto theRect = &lightmap_rectchange[lmap];
...
void R_RebuildAllLightmaps()
{
	...
	for (i = 1; i < MAX_MODELS; i++)
	{
		if (!((mod = cl.model_precache[i])))
			continue;
		auto fa = &mod->surfaces[mod->firstmodelsurface];
		for (auto j = 0; j < mod->nummodelsurfaces; j++ , fa++)
		{
