...
void Sbar_DrawScrollString(int x, int y, int width, char* str)
{
	...
	int len = strlen(str) * 8 + 40;
	auto ofs = static_cast<int>(realtime * 30) % len;
...
void Sbar_DrawFrags()
{
	...
	// draw the text
	auto numscores = min (scoreboardlines, 4);

	for (i = 0 , x = 184; i < numscores; i++ , x += 32)
	{
		auto s = &cl.scores[fragsort[i]];
		if (!s->name[0])
			continue;

		// top color
		auto color = s->colors & 0xf0;
...
void Sbar_Draw()
{
	...
	auto w = CLAMP (320.0f, scr_sbarscale.value * 320.0f, (float)glwidth);
	...
	GL_SetCanvas(canvastype::CANVAS_SBAR); //johnfitz
...
void Sbar_DeathmatchOverlay()
{
	...
	auto x = 80; //johnfitz -- simplified becuase some positioning is handled elsewhere
	auto y = 40;
	for (auto i = 0; i < l; i++)
	{
		auto k = fragsort[i];
...
void Sbar_MiniDeathmatchOverlay()
{
	...
	float scale = CLAMP (1.0, scr_sbarscale.value, (float)glwidth / 320.0); //johnfitz
...
void Sbar_IntermissionOverlay()
{
	...
	auto pic = Draw_CachePic("gfx/complete.lmp");
	Draw_Pic(64, 24, pic);
