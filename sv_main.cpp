...
void SV_Init()
{
	...
	Cvar_RegisterVariable(&sv_maxvelocity, nullptr);
	Cvar_RegisterVariable(&sv_gravity, nullptr);
	Cvar_RegisterVariable(&sv_friction, nullptr);
	Cvar_RegisterVariable(&sv_edgefriction, nullptr);
	Cvar_RegisterVariable(&sv_stopspeed, nullptr);
	Cvar_RegisterVariable(&sv_maxspeed, nullptr);
	Cvar_RegisterVariable(&sv_accelerate, nullptr);
	Cvar_RegisterVariable(&sv_idealpitchscale, nullptr);
	Cvar_RegisterVariable(&sv_aim, nullptr);
	Cvar_RegisterVariable(&sv_nostep, nullptr);
	Cvar_RegisterVariable(&sv_altnoclip, nullptr); //johnfitz
...
void SV_AddToFatPVS(vec3_t org, mnode_t* node, model_t* worldmodel) //johnfitz -- added worldmodel as a parameter
{
	...
	while (true)
	{
		...
		if (node->contents < 0)
		{
			...
			{
				auto pvs = Mod_LeafPVS(reinterpret_cast<mleaf_t *>(node), worldmodel); //johnfitz -- worldmodel as a parameter
...
bool SV_VisibleToClient(edict_t* client, edict_t* test, model_t* worldmodel)
{
	...
	auto pvs = SV_FatPVS(org, worldmodel);

	for (auto i = 0; i < test->num_leafs; i++)
...
void SV_WriteEntitiesToClient(edict_t* clent, sizebuf_t* msg)
{
	...
	auto pvs = SV_FatPVS(org, sv.worldmodel);
	...
	for (auto e = 1; e < sv.num_edicts; e++ , ent = NEXT_EDICT(ent))
	{
		if (ent != clent) // clent is ALLWAYS sent
		{
			...
			if (sv_protocol == PROTOCOL_NETQUAKE && static_cast<int>(ent->v.modelindex) & 0xFF00)
				continue;
		...
		if (sv.protocol != PROTOCOL_NETQUAKE)
		{
			...
			if (bits & U_FRAME && static_cast<int>(ent->v.frame) & 0xFF00) bits |= U_FRAME2;
			if (bits & U_MODEL && static_cast<int>(ent->v.modelindex) & 0xFF00) bits |= U_MODEL2;
		...
		if (bits & U_FRAME2)
			MSG_WriteByte(msg, static_cast<int>(ent->v.frame) >> 8);
		if (bits & U_MODEL2)
			MSG_WriteByte(msg, static_cast<int>(ent->v.modelindex) >> 8);
		if (bits & U_LERPFINISH)
			MSG_WriteByte(msg, static_cast<byte>(Q_rint((ent->v.nextthink-sv.time)*255)));
		//johnfitz
	}
...
void SV_WriteClientdataToMessage(edict_t* ent, sizebuf_t* msg)
{
	...
	{
		auto other = PROG_TO_EDICT(ent->v.dmg_inflictor);
	...
	if (sv.protocol != PROTOCOL_NETQUAKE)
	{
		...
		if (static_cast<int>(ent->v.armorvalue) & 0xFF00) bits |= SU_ARMOR2;
		if (static_cast<int>(ent->v.currentammo) & 0xFF00) bits |= SU_AMMO2;
		if (static_cast<int>(ent->v.ammo_shells) & 0xFF00) bits |= SU_SHELLS2;
		if (static_cast<int>(ent->v.ammo_nails) & 0xFF00) bits |= SU_NAILS2;
		if (static_cast<int>(ent->v.ammo_rockets) & 0xFF00) bits |= SU_ROCKETS2;
		if (static_cast<int>(ent->v.ammo_cells) & 0xFF00) bits |= SU_CELLS2;
		if (bits & SU_WEAPONFRAME && static_cast<int>(ent->v.weaponframe) & 0xFF00) bits |= SU_WEAPONFRAME2;
	...
	if (bits & SU_ARMOR2)
		MSG_WriteByte(msg, static_cast<int>(ent->v.armorvalue) >> 8);
	if (bits & SU_AMMO2)
		MSG_WriteByte(msg, static_cast<int>(ent->v.currentammo) >> 8);
	if (bits & SU_SHELLS2)
		MSG_WriteByte(msg, static_cast<int>(ent->v.ammo_shells) >> 8);
	if (bits & SU_NAILS2)
		MSG_WriteByte(msg, static_cast<int>(ent->v.ammo_nails) >> 8);
	if (bits & SU_ROCKETS2)
		MSG_WriteByte(msg, static_cast<int>(ent->v.ammo_rockets) >> 8);
	if (bits & SU_CELLS2)
		MSG_WriteByte(msg, static_cast<int>(ent->v.ammo_cells) >> 8);
	if (bits & SU_WEAPONFRAME2)
		MSG_WriteByte(msg, static_cast<int>(ent->v.weaponframe) >> 8);
...
void SV_CreateBaseline()
{
	...
	for (auto entnum = 0; entnum < sv.num_edicts; entnum++)
	{
...
void SV_SpawnServer(char* server)
{
	...
	Cvar_SetValue("skill", static_cast<float>(current_skill));
