...
void SV_CheckAllEnts()
{
	...
	for (auto e = 1; e < sv.num_edicts; e++ , check = NEXT_EDICT(check))
	{
		if (check->free)
			continue;
...
bool SV_RunThink(edict_t* ent)
{
	...
	auto oldframe = ent->v.frame; //johnfitz
	...
	{
		auto i = Q_rint((ent->v.nextthink-thinktime)*255);
...
int SV_FlyMove(edict_t* ent, float time, trace_t* steptrace)
{
	...
	for (auto bumpcount = 0; bumpcount < numbumps; bumpcount++)
	{
...
void SV_PushMove(edict_t* pusher, float movetime)
{
	...
	moved_edict = reinterpret_cast<edict_t **>(Hunk_Alloc(sv.num_edicts * sizeof(edict_t *)));
	moved_from = reinterpret_cast<vec3_t*>(Hunk_Alloc(sv.num_edicts * sizeof(vec3_t)));
	//johnfitz
	...
	for (auto e = 1; e < sv.num_edicts; e++ , check = NEXT_EDICT(check))
	{
		...
		if (! (static_cast<int>(check->v.flags) & FL_ONGROUND
		...
		if (check->v.movetype != MOVETYPE_WALK)
			check->v.flags = static_cast<int>(check->v.flags) & ~FL_ONGROUND;
...
bool SV_CheckWater(edict_t* ent)
{
	...
	auto cont = SV_PointContents(point);
...
void SV_WallFriction(edict_t* ent, trace_t* trace)
{
	...
	auto d = DotProduct (trace->plane.normal, forward);
...
void SV_WalkMove(edict_t* ent)
{
	...
	auto clip = SV_FlyMove(ent, host_frametime, &steptrace);
...
void SV_Physics()
{
	...
	//
	ent = sv.edicts;
	for (auto i = 0; i < sv.num_edicts; i++ , ent = NEXT_EDICT(ent))
	{
		if (ent->free)
			continue;
