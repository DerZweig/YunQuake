...
hull_t* SV_HullForEntity(edict_t* ent, vec3_t mins, vec3_t maxs, vec3_t offset)
{
	...
	if (ent->v.solid == SOLID_BSP)
	{
		...
		auto model = sv.models[static_cast<int>(ent->v.modelindex)];

		if (!model || model->type != modtype_t::mod_brush)
...
areanode_t* SV_CreateAreaNode(int depth, vec3_t mins, vec3_t maxs)
{
	...
	auto anode = &sv_areanodes[sv_numareanodes];
	sv_numareanodes++;
...
void SV_LinkEdict(edict_t* ent, qboolean touch_triggers)
{
	...
	//
	if (static_cast<int>(ent->v.flags) & FL_ITEM)
	{
		ent->v.absmin[0] -= 15;
		ent->v.absmin[1] -= 15;
		ent->v.absmax[0] += 15;
...
{
	auto cont = SV_HullPointContents(&sv.worldmodel->hulls[0], 0, p);
...
void SV_ClipToLinks(areanode_t* node, moveclip_t* clip)
{
	...
	for (l = node->solid_edicts.next; l != &node->solid_edicts; l = next)
	{
		...
		if (static_cast<int>(touch->v.flags) & FL_MONSTER)
