...
static DIOBJECTDATAFORMAT rgodf[] = {
	...
	{nullptr, FIELD_OFFSET(MYDATA, bButtonA), DIDFT_BUTTON | DIDFT_ANYINSTANCE, 0,},
	{nullptr, FIELD_OFFSET(MYDATA, bButtonB), DIDFT_BUTTON | DIDFT_ANYINSTANCE, 0,},
	{nullptr, FIELD_OFFSET(MYDATA, bButtonC), 0x80000000 | DIDFT_BUTTON | DIDFT_ANYINSTANCE, 0,},
	{nullptr, FIELD_OFFSET(MYDATA, bButtonD), 0x80000000 | DIDFT_BUTTON | DIDFT_ANYINSTANCE, 0,},
};
...
qboolean IN_InitDInput()
{
	...
	hr = g_pMouse->SetCooperativeLevel(mainwindow, DISCL_EXCLUSIVE | DISCL_FOREGROUND);
...
void IN_MouseEvent(int mstate)
{
	if (mouseactive && !dinput)
	{
		...
		for (auto i = 0; i < mouse_buttons; i++)
		{
			if (mstate & 1 << i &&
				!(mouse_oldbuttonstate & 1 << i))
			{
				Key_Event(K_MOUSE1 + i, qtrue);
			}

			if (!(mstate & 1 << i) &&
				mouse_oldbuttonstate & 1 << i)
			{
				Key_Event(K_MOUSE1 + i, qfalse);
			}
		}
...
void IN_MouseMove(usercmd_t* cmd)
{
	...
	if (lookstrafe.value && in_mlook.state & 1)
...
void Joy_AdvancedUpdate_f()
{
	...
	if (joy_advanced.value == 0.0)
	{
		...
		dwAxisMap[JOY_AXIS_X] = _ControlList::AxisTurn;
		// dwControlMap[JOY_AXIS_X] = JOY_ABSOLUTE_AXIS;
		dwAxisMap[JOY_AXIS_Y] = _ControlList::AxisForward;
...
void IN_Commands()
{
	...
	for (i           = 0; i < joy_numbuttons; i++)
	{
		if (buttonstate & 1 << i && !(joy_oldbuttonstate & 1 << i))
		{
			...
			Key_Event(key_index + i, qtrue);
		}

		if (!(buttonstate & 1 << i) && joy_oldbuttonstate & 1 << i)
		{
			key_index = i < 4 ? K_JOY1 : K_AUX1;
			Key_Event(key_index + i, qfalse);
		}
	}
	...
	if (joy_haspov)
	{
		...
		for (i = 0; i < 4; i++)
		{
			...
			{
				Key_Event(K_AUX29 + i, qtrue);
			}

			if (!(povstate & 1 << i) && joy_oldpovstate & 1 << i)
			{
				Key_Event(K_AUX29 + i, qfalse);
			}
		}
...
void IN_JoyMove(usercmd_t* cmd)
{
	...
	for (auto i = 0; i < JOY_MAX_AXES; i++)
	{
		...
		auto fAxisValue = static_cast<float>(*pdwRawValue[i]);
		// move centerpoint to zero
		fAxisValue -= 32768.0;

		if (joy_wwhack2.value != 0.0)
		{
			if (dwAxisMap[i] == _ControlList::AxisTurn)
			{
