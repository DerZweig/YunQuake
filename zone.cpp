...
void Z_ClearZone(memzone_t* zone, int size)
{
	...
	zone->blocklist.next = zone->blocklist.prev = block = reinterpret_cast<memblock_t *>(reinterpret_cast<byte *>(zone) + sizeof(memzone_t));
...
void* Z_TagMalloc(int size, int tag)
{
	...
	{ // there will be a free fragment after the allocated block
		auto cur = reinterpret_cast<memblock_t *>(reinterpret_cast<byte *>(base) + size);
		cur->size = extra;
		cur->tag = 0; // free block
		cur->prev = base;
		cur->id = ZONEID;
		cur->next = base->next;
		cur->next->prev = cur;
...
void Cache_Move(cache_system_t* c)
{
	...
	auto cur = Cache_TryAlloc(c->size, true);
	if (cur)
	{
		//		Con_Printf ("cache_move ok\n");

		Q_memcpy(cur + 1, c + 1, c->size - sizeof(cache_system_t));
		cur->user = c->user;
		Q_memcpy(cur->name, c->name, sizeof cur->name);
		Cache_Free(c->user, false); //johnfitz -- added second argument
		cur->user->data = static_cast<void *>(cur + 1);
	}
	else
	{
...
cache_system_t* Cache_TryAlloc(int size, bool nobottom)
{
	...
	if (!nobottom && cache_head.prev == &cache_head)
	{
		...
		cur = reinterpret_cast<cache_system_t *>(hunk_base + hunk_low_used);
		memset(cur, 0, sizeof*cur);
		cur->size = size;

		cache_head.prev = cache_head.next = cur;
		cur->prev = cur->next = &cache_head;
	...
	do
	{
		...
		{
			if (reinterpret_cast<byte *>(cs) - reinterpret_cast<byte *>(cur) >= size)
			{ // found space
				memset(cur, 0, sizeof*cur);
				cur->size = size;
		...
		// continue looking
		cur = reinterpret_cast<cache_system_t *>(reinterpret_cast<byte *>(cs) + cs->size);
		cs = cs->next;
	}
	while (cs != &cache_head);

	// try to allocate one at the very end
	if (hunk_base + hunk_size - hunk_high_used - reinterpret_cast<byte *>(cur) >= size)
	{
		memset(cur, 0, sizeof*cur);
		cur->size = size;

		cur->next = &cache_head;
		cur->prev = cache_head.prev;
		cache_head.prev->next = cur;
		cache_head.prev = cur;
...
void Cache_Free(cache_user_t* c, bool freetextures) //johnfitz -- added second argument
{
	...
	if (freetextures)
		TexMgr_FreeTexturesForOwner(reinterpret_cast<model_t *>(c + 1) - 1);
}
...
void* Cache_Alloc(cache_user_t* c, int size, char* name)
{
	...
	while (true)
	{
		auto cs = Cache_TryAlloc(size, false);
		if (cs)
		{
